...
func change_music(type: MusicEnum, animation_duration: float):
	...
	while (time < animation_duration):
		...
		if (use_second_player):
			volume_db = linear_to_db(lerp(music_volume, 0.0, time / animation_duration));
			second_player.volume_db = linear_to_db(lerp(0.0, music_volume, time / animation_duration));
		else:
			second_player.volume_db = linear_to_db(lerp(music_volume, 0.0, time / animation_duration));
			volume_db = linear_to_db(lerp(0.0, music_volume, time / animation_duration));
